...
To view team member’s calendars, click on the plus symbol next to “Other calendars” on the left hand side of your calendar and select “Subscribe to calendar.” There is a full list of team emails in the [KRAI Team Roster](https://docs.google.com/spreadsheets/d/1hjFkcU68e7AMs-ueEYR82idqi3-dMJk-p1VRd5_YZV8/edit#gid=0) document. 
...
Workspace: multiple bases can be organized within a workspace. For example, we have an KRAI workspace and individual project bases. 

### Getting to Know the KRAI Workspace

Within the KRAI workspace, we have the following bases:

* KRAI Projects and Pipeline
* Project Template
* Project-specific bases

![](images/airtable_workspace.png)

The `KRAI Projects and Pipeline` base contains overview information about current and potential projects. It is organized into the following tabs: Team Directory, Projects (current and archived), Deliverables (as outlined in the scope of work), and In the Pipeline (potential projects and their current stage). 
...
We have all experienced the moment where we can’t remember where a Doc is. To help solve this problem, we created an KRAI Shared OneDrive Folder, which is a centralized space for all of our documents to live within a shared file structure. Unlike files in My Drive (your personal OneDrive account), files within the Shared Drive belong to the team instead of an individual. So even if people leave, the files stay exactly where they are and aren’t lost with that person’s account. The structure of Shared Drive is detailed in Section 2. 
...
If you want to share files with people outside of the KRAI Shared Drive, that is still possible. Within a document, click on Share in the upper right hand corner. From there are two options: either add people’s emails or create a shared link. 
...
Since most of our projects at KRAI involve code, we use Git to track changes made to our code and faciliate collaboration by merging changes made by others, and GitHub to organize, share, and backup our code. 

This section provides a brief overview of how Git and Github work, how to install them on your computer (and how to join the KRAI GitHub page), and some general guidelines for how to use GitHub to organize code associated with KRAI projects. 
...
If you're new to using Git and GitHub, the two tutorials listed above are a great place to start since NCEAS and KRAI often operate in a similar way. Additionally, see the Software Carpentry's lesson for the [Git novice](http://swcarpentry.github.io/git-novice/). If you primarily use (or will use) R for coding, Jenny Bryan also has an **excellent** tutorial specifically about how to integrate Git and GitHub with R:
...
- repository ("repo"): a collection of files pertaining to the same project, document, goal, etc. Generally there's a single repository for each project at KRAI containing all of the code associated with that project. This repository can be organized with multiple folders and subfolders.  
...
Most of tutorials listed above include detailed instructions on how to install Git and GitHub. The short version (and steps specific to getting incorporated with the KRAI GitHub page) are listed below. For more detailed instructions, please refer to the tutorials listed above. 
...
Good news! KRAI qualifies as an educational organization through the GitHub Education program, and as a UCSB staff member you qualify for the individual educational discount. So, once you've signed up for a free account on GitHub... 
...
3. Send Erin O'Reilly a Slack message (or an email if you must... eoreilly@ucsb.edu) with your new GitHub username so you can be added to the KRAI GitHub page!

![KRAI GitHub page](images/github_KRAI.png) 

The KRAI GitHub page is where the repositories for all KRAI projects live (more on this later), and once you are a member of the organization you will be able to create new public and private repositories that appear here (as well as on your personal page). 
...
### General Guidelines for using GitHub at KRAI

In general, each KRAI project should have its own repository. There may be some cases in which multiple repositories may be associated with the same project, but this should be avoided if possible. The project repository should be created within the KRAI GitHub page (exceptions may exist for example if a partner organization requires that the project repository be created within their organization's GitHub page). 
...
Since many previous (and ongoing) projects were created within the personal GitHub pages of KRAI members, the ownership of these repositories should be transferred to the KRAI GitHub page at the conclusion of the project if possible.
...
KRAI uses Zotero to collaboratively collect, organize, and cite publications. We organize publications into Group Libraries which can be at the project or paper level. Our account – `KRAI-ucsb` – has unlimited storage and there is no limit to the number of people you can add to a library. In Zotero, storage for a library only counts against the owner, so **the KRAI account must be the owner of the library for access to unlimited storage.** 

### Making KRAI the Owner of a Library

If creating a new group library, email Erin (eoreilly@ucsb.edu) and she can directly create the library under the KRAI account. 

If you are the owner of an existing library and would like to transfer ownership to KRAI, first invite `KRAI-ucsb` to be a member of the library. Once that invitation has been accepted, then go to "Group Settings" for the library and in the bottom right of the page, select transfer ownership to the KRAI account.
