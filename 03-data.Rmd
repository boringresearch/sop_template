...
Data should be managed and shared properly to make it useful in research, to promote transparency and facilitate reproducibility, and to ensure the credibility of research. This includes such practices as transforming data into a tidy format, storing data in open file formats, and providing data documentation. **All <UNK> data, unless restricted, should be stored in the <UNK> Shared Drive (location dependent on type of data, see Section 2.2.3).**
...
All style guides are opinionated and while they all intend to make code easier to read and write, some decisions are arbitrary. Thus, instead of us at <UNK> creating our own, we want to adopt the "proven and tested" [Tidyverse style guide](https://style.tidyverse.org/). This styles guide has good advice not only for naming files but also for naming functions, objects, and general best coding practices. We encourage all <UNK> members to read the guide and work towards adopting it. 
...
All readme metadata files are stored in the folder that contains the data file in the [<UNK> Data Directory](https://docs.google.com/spreadsheets/d/1lCzpP1X0qPQrqDzVBi78XJQVUumbcQqRck60OkHtANk/edit#gid=0).

## Data Directory 

### <UNK> Data Directory

All <UNK> data is stored in subfolders of the Data folder on the <UNK> Team Drive (`<UNK>/data`). To document these data, we use the [<UNK> Data Directory](https://docs.google.com/spreadsheets/d/1lCzpP1X0qPQrqDzVBi78XJQVUumbcQqRck60OkHtANk/edit#gid=0) that includes key, standardized information from each readme metadata file. Every data file in the `<UNK>/data` folder has a record (row) in the <UNK> Data Directory. The <UNK> Data Directory file contains two sheets: (1) Data directory (the record and standardized documentation for each data file); (2) Metadata (information needed to populate the Data Directory, i.e. the meta-metadata)
...
* These agreements should go through UCSB’s [Office of Technology & Industry Alliances (TIA)](https://tia.ucsb.edu).  
    ...
    + CC <UNK>'s Amanda Kelley (aekelley@ucsb.edu) on all emails relating to the DUA or NDA.  
...
* <UNK> is generally happy to work with partners to determine the most appropriate method for data storage. Some partners may have specific data storage requirements that will be laid out in the DUA or NDA.   
* If the DUA or NDA does not have specific data storage requirements, we recommend one of the following three approaches depending on how sensitive the data are:  
    + For data that are *not* confidential or sensitive, data should be stored in a project-specific directory on the <UNK> Team Drive. Only <UNK> PIs and full-time <UNK> staff will have default access to this data directory. Any additional access for postdocs, students, or other external collaborators will only be granted on an as-needed basis and only after the collaborator has read the DUA or NDA. This option is used for the vast majority of <UNK> projects.
...
* High-level metadata for all datasets should be added to the _<UNK>_data_directory. See this section in the <UNK> SOP for further description of the <UNK> data directory. This will help <UNK> be internally transparent in how we are using data for different projects, even if the entire group doesn’t have access to the data.  
